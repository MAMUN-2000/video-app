...
function VideoList() {
  ...
  const { filterTags, search } = useSelector((state) => state.filterVideo);

  useEffect(() => {
    dispatch(fetchVideos({ filterTags, search }));
  }, [dispatch, filterTags, search]);
  ...
  if (isLoading) {
    ...
    content = (
      <h1 className="text-uppercase text-2xl text-red-600">no video found !</h1>
    );
  }
